
// This is a fake key it will not work with pokemon go
// Please do not contact me to get a genuine key, these 
// need to be extracted from a pogo+ which can be obtained
// second hand on ebay, other "genuine new" ones are clones
// all with the same key.

#include <stdint.h>

uint8_t MAC[6] = {0x01, 0x02, 0x02, 0x03, 0x04, 0x05};

uint8_t BLOB[256] = {
	
};

uint8_t DEVICE_KEY[16] = {
	0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
	0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
	0x0c, 0x0d, 0x0e
};

